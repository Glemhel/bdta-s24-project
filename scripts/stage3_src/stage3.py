...
from modeling import make_model1, make_model2, make_model3, make_model4, train_model_cv, evaluate_model, print_and_save_model
...
train = processed_dataset.sampleBy('label', fractions={1: 0.8, 2: 0.8, 3: 0.8, 4: 0.8}, seed=42)
...
evaluatorWeightedF = MulticlassClassificationEvaluator(labelCol="label", predictionCol="prediction", weightCol="weight", metricName="weightedFMeasure")
evaluatorAccuracy = MulticlassClassificationEvaluator(labelCol="label", predictionCol="prediction", metricName="accuracy")
evaluatorF1 = MulticlassClassificationEvaluator(labelCol="label", predictionCol="prediction")
evaluatorWeightedPrecision = MulticlassClassificationEvaluator(labelCol="label", predictionCol="prediction", weightCol="weight", metricName="weightedPrecision")
evaluatorWeightedRecall = MulticlassClassificationEvaluator(labelCol="label", predictionCol="prediction", weightCol="weight", metricName="weightedRecall")
evaluators = [evaluatorWeightedF, evaluatorAccuracy, evaluatorF1, evaluatorWeightedPrecision, evaluatorWeightedRecall]
...
df_comparison = spark.createDataFrame(comparison_list,
    ["model", "Weighted F-score", "Accuracy", "F1", "Weighted Precision", "Weighted Recall"]
)
df_comparison.show(truncate=False)

# Save it to HDFS
df_comparison.coalesce(1)\
    .write\
    .mode("overwrite")\
    .format("csv")\
    .option("sep", ",")\
...
run_os_command("hdfs dfs -cat project/output/evaluation.csv/*.csv > output/evaluation.csv")
