...
from pyspark.sql.functions import (
    dayofweek, dayofmonth, hour, month, year, sin, cos, col,
    unix_timestamp, when
)
...
class CategoryClipTransformer(Transformer, HasInputCol, HasOutputCol, DefaultParamsReadable, DefaultParamsWritable):
    """
    ...
    """
    input_col = Param(Params._dummy(), "input_col", "Name of the input column.", typeConverter=TypeConverters.toString)
    output_col = Param(Params._dummy(), "output_col", "Name of the output column.", typeConverter=TypeConverters.toString)
    threshold = Param(Params._dummy(), "threshold", "Threshold value for clipping.", typeConverter=TypeConverters.toInt)

    def __init__(self, input_col: str = "input", output_col: str = "output", threshold: int = 20):
        """
    ...
    def set_params(self, input_col: str = "input", output_col: str = "output", threshold: int = 20):
        """
    ...
    def _transform(self, df: DataFrame):
        ...
        return df.withColumn(output_col, when(col(input_col) >= threshold, threshold).otherwise(col(input_col)))
...
def with_weight_column(df):
    ...
    class_proportions_dict = {row["label"]: row["proportion"] for row in class_proportions.collect()}
    ...
    for label, weight in weights.items():
        df_copy = df_copy.withColumn("weight", when(col("label") == label, weight).otherwise(col("weight")))
...
def preprocess_dataset(dataset):
    ...
    data = dataset.select(
        ...
        'weather_timestamp', 'temperature_f', 'humidity_percent',
        'pressure_in', 'visibility_mi',
    ...
    data = data.withColumn('month_sin', sin(2 * math.pi * month(col('start_time')) / 12))
    data = data.withColumn('month_cos', cos(2 * math.pi * month(col('start_time')) / 12))
    data = data.withColumn('dayofweek_sin', sin(2 * math.pi * dayofweek(col('start_time')) / 7))
    data = data.withColumn('dayofweek_cos', cos(2 * math.pi * dayofweek(col('start_time')) / 7))
    data = data.withColumn('day_sin', sin(2 * math.pi * dayofmonth(col('start_time')) / 31))
    data = data.withColumn('day_cos', cos(2 * math.pi * dayofmonth(col('start_time')) / 31))
    ...
    data = data.withColumn('weather_time',
        unix_timestamp(col("start_time")) - unix_timestamp(col("weather_timestamp"))
    )

    # Coordinates-related transformations
    data = data.withColumn('ecef_x', ecef_udf_currying('x')(col('start_lat'), col('start_lng')))
    data = data.withColumn('ecef_y', ecef_udf_currying('y')(col('start_lat'), col('start_lng')))
    data = data.withColumn('ecef_z', ecef_udf_currying('z')(col('start_lat'), col('start_lng')))

    # Drop unnecessary columns
    data = data.drop('start_lat', 'start_lng', 'start_time', 'weather_timestamp', 'description', 'distance_mi')

    # Define categorical and numerical columns
    categoricalCols = ['street', 'city', 'county', 'state', 'zipcode',
                       'timezone', 'airport_code',
                       'wind_direction', 'weather_condition']
    numericalCols = ['ecef_x', 'ecef_y', 'ecef_z',
                     'year', 'hour_sin', 'hour_cos', 'day_sin', 'day_cos',
                     'month_sin', 'month_cos', 'dayofweek_sin', 'dayofweek_cos',
                     'temperature_f', 'humidity_percent',
                     'pressure_in', 'visibility_mi', 'wind_speed_mph', 'weather_time',
                     'amenity', 'bump', 'crossing', 'give_way', 'junction', 'no_exit',
                     'railway', 'roundabout', 'station', 'stop', 'traffic_calming',
                     'traffic_signal',
                     'sunrise_sunset', 'civil_twilight', 'nautical_twilight',
    ...
    indexers = [
        StringIndexer(inputCol=c, outputCol=f"{c}_indexed",
                      handleInvalid="error").fit(data) for c in categoricalCols
    ]

    # Clip categories
    clippers = [
        CategoryClipTransformer(input_col=indexer.getOutputCol(),
                                output_col=f"{indexer.getOutputCol()}_clipped", threshold=20).fit(data)
        for indexer in indexers
    ]

    # Encode the strings using One Hot encoding
    encoders = [OneHotEncoder(inputCol=clipper.get_output_col(),
                              outputCol=f"{clipper.get_output_col()}_encoded").fit(data)
        for clipper in clippers
    ]

    # Assemble features
    assembler = VectorAssembler(inputCols=[encoder.getOutputCol()for encoder in encoders] + numericalCols,
                                outputCol="features").fit(data)
